...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def gabriel_api_error_handler(
        request: Request, exc: GabrielAPIError
    ) -> JSONResponse:
        return JSONResponse(
    ...
    async def handler_not_found_error(
    ...
    async def command_validation_error(
        request: Request, exc: CommandValidationError
    ) -> JSONResponse:
        return JSONResponse(
            status_code=422,
            content=_error_body(request, str(exc)),
    ...
    async def handler_execution_error(
        request: Request, exc: HandlerExecutionError
    ) -> JSONResponse:
        return JSONResponse(
            status_code=500,
            content=_error_body(request, str(exc)),

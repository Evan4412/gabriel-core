...
def test_login_issues_signed_token_and_cookie(client):
    response = client.post(
        "/api/v1/auth/login", json={"method": "dev", "userId": "u_alice"}
    )
    assert response.status_code == 200
    body = response.json()
...
def test_login_unknown_method_is_bad_request(client):
    response = client.post(
        "/api/v1/auth/login", json={"method": "saml", "userId": "u_alice"}
    )
...
def test_session_endpoint_uses_cookie_from_dev_login(client):
    assert (
        client.post("/api/v1/auth/dev/login", json={"userId": "u_alice"}).status_code
        == 200
    )

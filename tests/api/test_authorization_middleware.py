...
from gabriel.api.middleware.authorization import (
    _derive_action,
...
def test_derive_action_maps_http_method_and_domain(client):
    ...
    request = client.build_request(
        "POST", "/api/v1/agents/grn:acme:agent/bot:1/execute"
    )
...
def test_authorization_middleware_allows_and_emits_audit_event(
    client, make_auth_headers
):
    correlation_id = str(uuid4())
    headers = make_auth_headers(correlation_id=correlation_id)
    before = len(
        client.app.state.gateway_state.event_store.events_by_type("peel_evaluation")
    )

    response = client.get("/api/v1/memory", headers=headers)

    assert response.status_code == 200
    after_events = client.app.state.gateway_state.event_store.events_by_type(
        "peel_evaluation"
    )
...
def test_authorization_middleware_denies_and_emits_audit_event(
    client, make_auth_headers
):
    correlation_id = str(uuid4())
...
def test_audit_log_query_supports_principal_and_decision_filters(
    client, make_auth_headers
):
    ...
    assert all(
        item["principal_id"] == "principal://acme/user/alice-audit"
